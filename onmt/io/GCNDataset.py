...
class GCNDataset(ONMTDatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
                 label_examples_iter, node1_examples_iter, node2_examples_iter, title_examples_iter, types_examples_iter,
                 morph_iter='',
        ...
        if tgt_examples_iter is not None:
            if morph_iter == '':
                examples_iter = (self._join_dicts(src, label, node1, node2, tgt, title, types) for src, label, node1, node2, tgt, title, types in
            ...
            examples_iter = (self._join_dicts(src, label, node1, node2, title, types) for src, label, node1, node2, title, types in
                             zip(src_examples_iter, label_examples_iter, node1_examples_iter,
                                 node2_examples_iter, title_examples_iter, types_examples_iter))
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        def make_src(data, vocab, is_train=True):
        ...
        def make_tgt(data, vocab, is_train=True):
        ...
        # Adding title and types (check if needed here!)
        fields["title"] = torchtext.data.Field(
            use_vocab=True,  # tensor_type=torch.LongTensor,
            init_token=BOS_WORD, eos_token=EOS_WORD,
            pad_token=PAD_WORD,
            include_lengths=True,
            sequential=True)
        fields["types"] = torchtext.data.Field(
            use_vocab=True,  # tensor_type=torch.LongTensor,
            sequential=True)
