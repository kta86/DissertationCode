...
class GCNLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if in_arcs:
            ...
            nn.init.xavier_normal_(self.V_in)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja

            self.b_in = Parameter(torch.Tensor(num_labels, self.num_units))
            nn.init.constant_(self.b_in, 0)  # Changing from nn.init.constant() as it is deprecated - Katja

            if self.use_gates:
                self.V_in_gate = Parameter(torch.Tensor(self.num_inputs, 1))
                nn.init.xavier_normal_(self.V_in_gate)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja
                self.b_in_gate = Parameter(torch.Tensor(num_labels, 1))
                nn.init.constant_(self.b_in_gate, 1)  # Changing from nn.init.constant() as it is deprecated - Katja

        if out_arcs:
            self.V_out = Parameter(torch.Tensor(self.num_inputs, self.num_units))
            nn.init.xavier_normal_(self.V_out)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja

            self.b_out = Parameter(torch.Tensor(num_labels, self.num_units))
            nn.init.constant_(self.b_out, 0)  # Changing from nn.init.constant() as it is deprecated - Katja

            if self.use_gates:
                self.V_out_gate = Parameter(torch.Tensor(self.num_inputs, 1))
                nn.init.xavier_normal_(self.V_out_gate)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja
                self.b_out_gate = Parameter(torch.Tensor(num_labels, 1))
                nn.init.constant_(self.b_out_gate, 1)  # Changing from nn.init.constant() as it is deprecated - Katja

        self.W_self_loop = Parameter(torch.Tensor(self.num_inputs, self.num_units))

        nn.init.xavier_normal_(self.W_self_loop)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja

        if self.use_gates:
            self.W_self_loop_gate = Parameter(torch.Tensor(self.num_inputs, 1))
            nn.init.xavier_normal_(self.W_self_loop_gate)  # Changing from nn.init.xavier_normal() as it is deprecated - Katja
