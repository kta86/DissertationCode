...
def delexicalisation(out_src, out_trg, category, properties_objects):
    ...
    with open(os.path.dirname(os.path.realpath(sys.argv[0])) + '/delex_dict.json', encoding='utf8') as data_file:
    ...
    for subject in delex_subj:
        clean_subj = ''.join(subject.replace('_', ' '))  # re.split('(\W)',   - Katja
    ...
    for pro, obj in sorted(properties_objects.items()):
        obj_clean = ''.join(obj.replace('_', ' ').replace('"', ''))  # re.split('(\W)',   - Katja
...
def create_source_target(b, options, dataset, delex=True):
   ...
   #     triples.reverse()
        ...
        out_src = ' '.join(re.split(triples))
        for lex in lexics:
            # separate punct signs from text
            out_trg = ' '.join(re.split(lex.lex))
            if delex:
...
def relexicalise(predfile, rplc_list, fileid, part='dev', lowercased=True):
    ...
    with open(part + '-agenda-all-notdelex.triple', 'r', encoding="utf-8") as f:
    ...
    with open(part + '-all-notdelex-source.triple', 'r', encoding="utf-8") as f:
