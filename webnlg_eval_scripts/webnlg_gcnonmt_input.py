...
# Adding in a tokenizer based on Yannis (current copy) - Katja (self removed)
def tokenize(raw_text, split=False, output_sent_delim=False):
    nlp = English()  # probably the wrong place for this!!
    nlp.add_pipe(nlp.create_pipe("sentencizer"))  # and this...
    # sentences_tokenized = [self.moses_word_tokenizer(sent) for sent in self.moses_sent_tokenizer([raw_text])]
    doc = nlp(raw_text)
    sentences_tokenized = [[token.text for token in sent] for sent in doc.sents]
    split_tokens = []
    for sent in sentences_tokenized:
        if output_sent_delim:
            sent.append("<s>")
        split_tokens += sent
    return split_tokens if split else ' '.join(split_tokens)
...
def create_source_target(b, options, dataset, delex=True, relex=False, doCategory=[], negraph=False, lowercased=True):
    ...
    for entr in b.entries:
        ...
        for lex in lexics:
            ...
            for triple in tripleset.triples:
                triples += tripleSep + ' ' + tokenize(triple.s) + ' | ' + triple.p + ' | ' + tokenize(triple.o) + ' '  # tokenize and spaces added - Katja
                tripleSep = "< TSP >"
    ...
    if not relex:
        ...
        with open(dataset + '-agenda-' + options + '-src-nodes.txt', 'w+', encoding='utf8') as f:
            f.write('\n'.join(source_nodes_out).lower() if (lowercased and not delex) else '\n'.join(source_nodes_out))
        with open(dataset + '-agenda-' + options + '-src-labels.txt', 'w+', encoding='utf8') as f:
            f.write('\n'.join(source_edges_out_labels))
        with open(dataset + '-agenda-' + options + '-src-node1.txt', 'w+', encoding='utf8') as f:
            f.write('\n'.join(source_edges_out_node1))
        with open(dataset + '-agenda-' + options + '-src-node2.txt', 'w+', encoding='utf8') as f:
            f.write('\n'.join(source_edges_out_node2))
        with open(dataset + '-agenda-' + options + '-tgt.txt', 'w+', encoding='utf8') as f:
            f.write('\n'.join(target_out).lower() if (lowercased and not delex) else '\n'.join(target_out))

    with open(dataset + '-agenda-' + options + '.triple', 'w+', encoding='utf8') as f:
        f.write('\n'.join(source_out))
    with open(dataset + '-agenda-' + options + '.lex', 'w+', encoding='utf8') as f:
        f.write('\n'.join(target_out).lower() if (lowercased and not delex) else '\n'.join(target_out))

    # Adding types and title - Katja
    with open(dataset + '-agenda-' + options + '-title.txt', 'w+', encoding='utf8') as f:
        f.write('\n'.join(title).lower() if (lowercased and not delex) else '\n'.join(title))
    with open(dataset + '-agenda-' + options + '-types.txt', 'w+', encoding='utf8') as f:
        f.write('\n'.join(types).lower() if (lowercased and not delex) else '\n'.join(types))
