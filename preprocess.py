...
def build_save_text_dataset_in_shards_gcn(src_corpus, tgt_corpus,
                                          ...
                                          node2_corpus, title_corpus, types_corpus, fields,
    ...
    node2_iter = onmt.io.ShardedTextCorpusIterator(
        ...
        "node2", opt.max_shard_size,
                assoc_iter=src_iter)
    title_iter = onmt.io.ShardedTextCorpusIterator(
    ...
    while not src_iter.hit_end():
        ...
        dataset = onmt.io.GCNDataset(
                ...
                node1_iter, node2_iter, title_iter, types_iter, mf_iter,
                src_iter.num_feats, tgt_iter.num_feats,
                src_seq_length=opt.src_seq_length,
                tgt_seq_length=opt.tgt_seq_length,
                dynamic_dict=opt.dynamic_dict)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if opt.data_type == 'text':
        ...
        if corpus_type == 'train':
            ...
            title_corpus = opt.train_title
            types_corpus = opt.train_types
            if opt.train_mf != '':
            ...
            title_corpus = opt.valid_title
            types_corpus = opt.valid_types
            if opt.valid_mf != '':
                mf_corpus = opt.valid_mf
        if opt.train_mf != '':
            return build_save_text_dataset_in_shards_gcn(
                src_corpus, tgt_corpus,
                label_corpus, node1_corpus, node2_corpus, title_corpus, types_corpus,
                fields, corpus_type, opt, mf_corpus)
        else:
            return build_save_text_dataset_in_shards_gcn(
                src_corpus, tgt_corpus,
                label_corpus, node1_corpus, node2_corpus, title_corpus, types_corpus,
